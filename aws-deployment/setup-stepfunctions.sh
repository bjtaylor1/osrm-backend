...
if aws lambda get-function --function-name osrm-swap-instances 2>/dev/null; then
  ...
  aws lambda create-function --function-name osrm-swap-instances --runtime python3.12 --role arn:aws:iam::$ACCOUNT_ID:role/lambda-osrm-role --handler osrm-swap-instances.lambda_handler --zip-file fileb://<(cd $SCRIPT_DIR && zip -q -r - osrm-swap-instances.py) --timeout 600 --region us-east-1
fi
