...
-- like bicycle, but with trunk and primary reduced desirability if 50mph maxspeed or more.
-- primary e.g. A1133, A616.
-- trunk e.g. A617, A623
-- high traffic light penalty to avoid towns with lots, but not too high to pointlessly skirt round the odd one
-- prefers flat like 'quickest'
...
function setup()
  ...
  return {
    ...
    bicycle_speeds = {
      ...
      residential = default_speed * 0.7, --tend to be fairly rough surfaced due to keep being dug up for faster and faster broadband
      unclassified = default_speed,
      living_street = default_speed * 0.7,
      road = default_speed,
...
function handle_bicycle_tags(profile,way,result,data)
  ...
  local trunk40reduction = 0.5 -- the A6097 is this
  local trunk50reduction = 0.3 -- e.g. the A617
  local trunk60reduction = 0.1 --fairly horrible
  local primary40reduction = 1 --not unpleasant really
  local primary50reduction = 0.85 --not that unpleasant
  local primary60reduction = 0.7 --e.g. A1133


  if result.forward_speed > 1 or result.forward_speed > 1 then
    -- not doing this in speed handler 'cos it's maxspeed dependent.
    -- e.g. trunk roads in towns that are typically 30mph are usually fine, but higher max speeds (even 40mph, e.g. the A6097) can be unpleasant.

    if data.highway == 'trunk' then

      -- forward
      if data.maxspeed_forward > 95 then --60mph
        result.forward_speed = result.forward_speed * trunk60reduction
      elseif data.maxspeed_forward > 78 then -- 50mph
        result.forward_speed = result.forward_speed * trunk50reduction
      elseif data.maxspeed_forward > 60 then --40mph
        result.forward_speed = result.forward_speed * trunk40reduction
      end
      
      -- backward
      if data.maxspeed_backward > 95 then --60mph
        result.backward_speed = result.backward_speed * trunk60reduction
      elseif data.maxspeed_backward > 78 then -- 50mph
        result.backward_speed = result.backward_speed * trunk50reduction
      elseif data.maxspeed_backward > 60 then --40mph
        result.backward_speed = result.backward_speed * trunk40reduction
      end
    elseif data.highway == 'primary' then

      -- forward
      if data.maxspeed_forward > 95 then --60mph
        result.forward_speed = result.forward_speed * primary60reduction
      elseif data.maxspeed_forward > 78 then -- 50mph
        result.forward_speed = result.forward_speed * primary50reduction
      elseif data.maxspeed_forward > 60 then --40mph
        result.forward_speed = result.forward_speed * primary40reduction
      end
      
      -- backward
      if data.maxspeed_backward > 95 then --60mph
        result.backward_speed = result.backward_speed * primary60reduction
      elseif data.maxspeed_backward > 78 then -- 50mph
        result.backward_speed = result.backward_speed * primary50reduction
      elseif data.maxspeed_backward > 60 then --40mph
        result.backward_speed = result.backward_speed * primary40reduction
      end
    end
  end
